/* -----------------------------------------------------------------------
   ...
   One implementation of the RPC contract in supabase/schema.sql, shared by
   every backend that is not Postgres: the browser demo (localStorage) and the
   Netlify function (Netlify Blobs). Keeping it in one place is why the two
   cannot drift apart.

   createStore(db, commit) -> handlers, operating on a plain object.
-------------------------------------------------------------------------*/
(function (root, factory) {
  if (typeof module === 'object' && module.exports) module.exports = factory();
  else root.AF_STORE = factory();
  ...
  function seed() {
    ...
    // one day each, not every day — otherwise every date looks identical and the
    // calendar seems to be showing stale data
    const link = [];
    const put = (day, time, names) => {
    ...
    put(5, '16:00', ['Zawad', 'Arifa']);      // Friday
    put(6, '16:50', ['Rahat']);               // Saturday
    put(1, '17:40', ['Itrat', 'Abaan']);      // Monday
    put(3, '16:00', ['Zaraan']);              // Wednesday
    put(3, '17:40', ['Zawad']);
